  ...
  var polyfill = function() {
    ...
    if (!Element.prototype.matches) {
      Element.prototype.matches = Element.prototype.matchesSelector || Element.prototype.mozMatchesSelector ||
        Element.prototype.msMatchesSelector || Element.prototype.oMatchesSelector || Element.prototype
        .webkitMatchesSelector || function(s) {
  ...
  */
  ...
  var config = {
    ...
    menus: ['head', 'bold', 'fontSize', 'fontName', 'italic', 'underline', 'strikeThrough', 'foreColor',
      'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo',
      'redo'
    ],
      ...
      var panel = new Panel(this, {
        ...
        tabs: [{
            ...
            tpl: '<div>\n                            <input id="' + inputTextId +
              ...
              '" placeholder="\u94FE\u63A5\u6587\u5B57"/></td>\n                            <input id="' +
              inputLinkId + '" type="text" class="block" value="' + link +
  ...
  */
  ...
  List.prototype = {
    ...
    tryChangeActive: function tryChangeActive(e) {
      var editor = this.editor;
      var $elem = this.$elem;
      if (editor.cmd.queryCommandState('insertUnOrderedList') || editor.cmd.queryCommandState(
          'insertOrderedList')) {
        this._active = true;
        $elem.addClass('w-e-active');
      } else {
  ...
  */
  ...
  Video.prototype = {
    ...
    _createPanel: function _createPanel() {
      ...
      var tabsConfig = [{
          title: '上传视频',
          tpl: '<div class="w-e-up-img-container"><div id="' + upTriggerVideoId +
            '" class="w-e-up-btn"><i class="w-e-icon-upload2"></i></div><div style="display:none;"><input id="' +
            upFileVideoId +
            '" type="file" multiple="multiple" accept="application/pdf,video/*"/></div></div>',
          events: [{
            // 触发选择图片
        ...
        }, // first tab end
        {
          ...
          title: '插入视频',
          // 模板
          tpl: '<div><input id="' + textValId +
            '" type="text" class="block" placeholder="\u683C\u5F0F\u5982\uFF1A<iframe src=... ></iframe>"/><div class="w-e-button-container"><button id="' +
            btnId + '" class="right">\u63D2\u5165</button></div></div>',
          // 事件绑定
          events: [{
            selector: '#' + btnId,
            type: 'click',
  ...
  */
  ...
  Image.prototype = {
    ...
    _createInsertPanel: function _createInsertPanel() {
      ...
      // 判断 tabs 的显示
      var tabsConfigResult = [];
      if ((config.uploadImgShowBase64 || config.uploadImgServer || config.customUploadImg) && window
        .FileReader) {
        // 显示“上传图片”
        tabsConfigResult.push(tabsConfig[0]);
      }
  ...
  */
  ...
  API.prototype = {
    ...
    saveRange: function saveRange(_range) {
      ...
      // 判断选区内容是否在不可编辑区域之内
      if ($containerElem.attr('contenteditable') === 'false' || $containerElem.parentUntil(
  ...
  UploadImg.prototype = {
    ...
    uploadImg: function uploadImg(files) {
      ...
      if (uploadImgServer && typeof uploadImgServer === 'string') {
        ...
        if (hooks.before && typeof hooks.before === 'function') {
          ...
          if (beforeResult && (typeof beforeResult === 'undefined' ? 'undefined' : _typeof(beforeResult)) ===
            'object') {
    ...
    uploadVideo: function uploadVideo(files) {
      ...
      arrForEach(files, function(file) {
        ...
        if (/\.(pdf|rm|rmvb|3gp|avi|mpeg|mpg|mkv|dat|asf|wmv|flv|mov|mp4|ogg|ogm)$/i.test(name) ===
          false) {
          // 后缀名不合法，不是视频
          errInfo.push('\u3010' + name + '\u3011\u4E0D\u662F\u56FE\u7247');
          return;
    ...
    insertLinkVideo: function insertLinkVideo(link) {
      ...
      editor.cmd.do('insertHTML', '<iframe src="' + link +
        '" style="width:650px;height: 366px" frameborder="0"></iframe>');
    }
