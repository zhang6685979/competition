      ...
      <el-col :span="8" v-for="(item, index) in dataList" :key="index" class="el-col">
        <div class="file-item">
          ...
          <div class="other-info">
            <h5>{{item.title}}</h5>
            <p>文件大小：{{(item.size/1024).toFixed(2)}}M | {{item.times}}次下载</p>
...
<style lang="scss" scoped>
  ...
  .file-item{
    ...
    padding:30px;
    display: flex;
    img{width: 48px;height:48px;display: inline-block;margin-right: 20px;}
    .other-info{
      h5{font-size: 16px;color: #000;}
      p{font-size: 14px;color: #707070;margin:10px 0;}
    }
  }
