      ...
      <vxe-toolbar :refresh="{query: refreshList}">
        <template #buttons>
          ...
          <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
            :disabled="$refs.competitionEquipmentTable && $refs.competitionEquipmentTable.getCheckboxRecords().length !== 1"
            plain>修改</el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" @click="del()"
            :disabled="$refs.competitionEquipmentTable && $refs.competitionEquipmentTable.getCheckboxRecords().length === 0"
            plain>删除</el-button>
        </template>
      </vxe-toolbar>
      <div style="height: calc(100% - 80px);">
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ref="competitionEquipmentTable" show-header-overflow show-overflow highlight-hover-row :menu-config="{}"
          ...
          <vxe-column field="title" sortable title="设备名称">
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" v-if="hasPermission('competition:competitionEquipment:edit')"
                @click="edit(scope.row.id)">{{scope.row.title}}</el-link>
              <el-link type="primary" :underline="false"
                v-else-if="hasPermission('competition:competitionEquipment:view')" @click="view(scope.row.id)">
                {{scope.row.title}}</el-link>
              <span v-else>{{scope.row.title}}</span>
            </template>
          </vxe-column>
          <vxe-column field="image" sortable title="设备图片">
          ...
          <vxe-column field="describe0" sortable title="设备描述">
          </vxe-column>
    ...
    methods: {
      ...
      del(id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.competitionEquipmentService.delete(ids).then(({
            data
          }) => {
