      ...
      <vxe-toolbar :refresh="{query: refreshList}">
        <template #buttons>
          ...
          <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
            :disabled="$refs.competitionNewsTable && $refs.competitionNewsTable.getCheckboxRecords().length !== 1"
            plain>修改</el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" @click="del()"
            :disabled="$refs.competitionNewsTable && $refs.competitionNewsTable.getCheckboxRecords().length === 0"
            plain>删除</el-button>
        </template>
      </vxe-toolbar>
      <div style="height: calc(100% - 80px);">
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ref="competitionNewsTable" show-header-overflow show-overflow highlight-hover-row :menu-config="{}"
          ...
          <vxe-column field="title" sortable title="新闻标题">
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" v-if="hasPermission('competition:competitionNews:edit')"
                @click="edit(scope.row.id)">{{scope.row.title}}</el-link>
              <el-link type="primary" :underline="false" v-else-if="hasPermission('competition:competitionNews:view')"
          ...
          <vxe-column field="iamge" sortable title="新闻图片">
          ...
          <vxe-column fixed="right" align="center" width="200" title="操作">
            <template slot-scope="scope">
              <el-button v-if="hasPermission('competition:competitionNews:view')" type="text" icon="el-icon-view"
                size="small" @click="view(scope.row.id)">查看</el-button>
              <el-button v-if="hasPermission('competition:competitionNews:edit')" type="text" icon="el-icon-edit"
                size="small" @click="edit(scope.row.id)">修改</el-button>
              <el-button v-if="hasPermission('competition:competitionNews:del')" type="text" icon="el-icon-delete"
    ...
    methods: {
      ...
      del(id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.competitionNewsService.delete(ids).then(({
            data
          }) => {
