      ...
      <vxe-toolbar :refresh="{query: refreshList}" export print custom>
        <template #buttons>
          ...
          <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
            :disabled="$refs.competitionPlayerTable && $refs.competitionPlayerTable.getCheckboxRecords().length !== 1"
            plain>修改</el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" @click="del()"
            :disabled="$refs.competitionPlayerTable && $refs.competitionPlayerTable.getCheckboxRecords().length === 0"
            plain>删除</el-button>
        </template>
      </vxe-toolbar>
        ...
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ...
          <vxe-column field="name" sortable title="姓名">
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" v-if="hasPermission('competition:competitionPlayer:edit')"
                @click="edit(scope.row.id)">{{scope.row.name}}</el-link>
              <el-link type="primary" :underline="false" v-else-if="hasPermission('competition:competitionPlayer:view')"
          ...
          <vxe-column field="mobile" sortable title="手机号">
          </vxe-column>
          <vxe-column field="email" sortable title="电子邮箱">
          </vxe-column>
          <vxe-column field="jobtitle" sortable title="职务">
          </vxe-column>
          <vxe-column field="role" sortable title="角色，0：裁判，1：领队，2：指导老师，3：参赛选手">
            <template slot-scope="scope">
              {{ $dictUtils.getDictLabel("jab_player_type", scope.row.role, '-') }}
            </template>
          </vxe-column>
          <vxe-column field="audited" sortable title="审核">
            <template slot-scope="scope">
              {{ $dictUtils.getDictLabel("yes_no", scope.row.audited, '-') }}
            </template>
          </vxe-column>
          <vxe-column field="cid" sortable title="比赛编号">
          </vxe-column>
          <vxe-column field="ctid" sortable title="关联赛事">
          </vxe-column>
    ...
    methods: {
      ...
      del(id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.competitionPlayerService.delete(ids).then(({
            data
          }) => {
