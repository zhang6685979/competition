      ...
      <vxe-toolbar :refresh="{query: refreshList}">
        <template #buttons>
          ...
          <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
            :disabled="$refs.competitionStyleTable && $refs.competitionStyleTable.getCheckboxRecords().length !== 1"
            plain>修改</el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" @click="del()"
            :disabled="$refs.competitionStyleTable && $refs.competitionStyleTable.getCheckboxRecords().length === 0"
            plain>删除</el-button>
        </template>
      </vxe-toolbar>
      <div style="height: calc(100% - 80px);">
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ref="competitionStyleTable" show-header-overflow show-overflow highlight-hover-row :menu-config="{}"
          ...
          <vxe-column field="title" sortable title="风采标题">
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" v-if="hasPermission('competition:competitionStyle:edit')"
                @click="edit(scope.row.id)">{{scope.row.title}}</el-link>
              <el-link type="primary" :underline="false" v-else-if="hasPermission('competition:competitionStyle:view')"
                @click="view(scope.row.id)">{{scope.row.title}}</el-link>
              <span v-else>{{scope.row.title}}</span>
            </template>
          </vxe-column>
          <vxe-column field="iamge" sortable title="风采图片">
