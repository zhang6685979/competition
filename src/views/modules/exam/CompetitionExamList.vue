<template>
  <div class="page">
      <vxe-toolbar :refresh="{query: refreshList}" export custom>
        <template #buttons>
          ...
          <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
            :disabled="$refs.competitionExamTable && $refs.competitionExamTable.getCheckboxRecords().length !== 1"
            plain>修改</el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" @click="del()"
            :disabled="$refs.competitionExamTable && $refs.competitionExamTable.getCheckboxRecords().length === 0"
            plain>删除</el-button>
        </template>
      </vxe-toolbar>
      <div style="height:400px">
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ref="competitionExamTable" show-header-overflow show-overflow highlight-hover-row :menu-config="{}"
          ...
          <vxe-column field="name" sortable title="考试名称">
          </vxe-column>
          <vxe-column field="starttime" sortable title="开始时间">
          </vxe-column>
          <vxe-column field="endtime" sortable title="结束时间">
          </vxe-column>
          <vxe-column field="rooms" sortable title="考场数量">
          </vxe-column>
          <vxe-column field="describe0" sortable title="考试描述">
          </vxe-column>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <CompetitionExamForm ref="competitionExamForm" :cid="cid" :crid="crid" @refreshDataList="refreshList">
    </CompetitionExamForm>
    <el-dialog title="执裁分配表" :visible.sync="dialogVisible" :append-to-body="true" width="80%">
      ...
      }" :list="distribution.distributed||[]" :surplusTeams="distribution.undistributedTeams||[]"
        :surplusReferees="distribution.undistributedReferees||[]" v-if="dialogVisible"></distributionTable>
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="dialogVisible = false">关闭</el-button>
        <el-button size="small" type="primary" @click="doSubmit()" v-noMoreClick>保存分配表</el-button>
      </span>
    </el-dialog>
    <el-dialog title="导入成绩" :close-on-click-modal="false" :append-to-body="true" v-dialogDrag :visible.sync="visible"
      width="60%">
    ...
    methods: {
      ...
      del(id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.competitionExamService.delete(ids).then(({
            data
          }) => {
