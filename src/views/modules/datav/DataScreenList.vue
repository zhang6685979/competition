    ...
    <el-form size="small" :inline="true" class="query-form" ref="searchForm" :model="searchForm"
      ...
      <el-form-item prop="categoryId">
        <SelectTree ref="category" :props="{
                    ...
                    }" placeholder="请选择大屏分类" size="small" url="/datav/dataScreenCategory/treeData"
          :value="searchForm.categoryId" :clearable="true" :accordion="true"
              ...
              <el-card style="margin: 8px" :body-style="{ padding: '0px' }" shadow="always">
                ...
                <h3 class="jp-card-label">
                  <el-row>
                    ...
                    <el-col :span="8">
                      <el-link :underline="false" class="el-icon-edit-outline right" @click="design(datascreen.id)"> 设计
                      </el-link>
                    </el-col>
                  </el-row>
                </h3>
                ...
                <ul class="jp-card-actions">
                  <li style="width: 25%">
                    <el-link :underline="false" @click="edit(datascreen.id)"><i class="el-icon-edit-outline"></i> 编辑
                    </el-link>
                  </li>
                  <li style="width: 25%">
                    <el-link :underline="false" @click="copy(datascreen.id)"><i class="el-icon-document-copy"></i> 复制
                    </el-link>
                  </li>
                  <li style="width: 25%">
    ...
    methods: {
      ...
      del(id) {
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.dataScreenService.delete(id).then(({
            data
          }) => {
      ...
      copy(id) {
        this.$confirm(`确定复制所选大屏吗?`, '提示', {
          ...
          this.loading = true
          this.dataScreenService.copy(id).then(({
            data
          }) => {
            this.$message.success(data)
            this.refreshList()
