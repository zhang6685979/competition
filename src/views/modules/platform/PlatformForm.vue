  ...
  <div>
    <el-dialog :title="title" :close-on-click-modal="false" v-dialogDrag :visible.sync="visible">
      <el-form :model="inputForm" size="small" ref="inputForm" v-loading="loading"
        ...
        <el-row :gutter="15">
          <el-col :span="24">
            <el-form-item label="平台名称" prop="title" :rules="[
          ...
          <el-col :span="24">
            <el-form-item label="平台图片" prop="image" :rules="[
              ...
              <el-upload ref="image" v-if="visible" list-type="picture-card" accept="image/png,image/jpeg"
                ...
                <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，建议图片尺寸为670*380</div>
              </el-upload>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="链接地址" prop="url" :rules="[
          ...
          <el-col :span="24">
            <el-form-item label="平台描述" prop="describe0" :rules="[
                 ]">
    ...
    methods: {
      init(method, id) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.platformService.queryById(this.inputForm.id).then(({
              data
            }) => {
      ...
      doSubmit() {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            this.platformService.save(this.inputForm).then(({
              data
            }) => {
              this.visible = false
