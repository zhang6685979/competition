<template>
     ...
     <div class="bg-white top">
        <vxe-toolbar :refresh="{query: refreshList}">
          <template #buttons>
            ...
            <el-button type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.carouselTable && $refs.carouselTable.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button  type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.carouselTable && $refs.carouselTable.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
        </vxe-toolbar>
    ...
    <vxe-pager
      ...
      @page-change="currentChangeHandle">
        ...
        <!-- 弹窗, 新增 / 修改 -->
    <CarouselForm  ref="carouselForm" @refreshDataList="refreshList"></CarouselForm>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 新增
      add () {
        this.$refs.carouselForm.init('add', '')
      },
      // 修改
      edit (id) {
        id = id || this.$refs.carouselTable.getCheckboxRecords().map(item => {
          return item.id
        })[0]
        this.$refs.carouselForm.init('edit', id)
      },
      // 查看
      view (id) {
        this.$refs.carouselForm.init('view', id)
      },
      // 删除
      del (id) {
        let ids = id || this.$refs.carouselTable.getCheckboxRecords().map(item => {
          return item.id
        }).join(',')
